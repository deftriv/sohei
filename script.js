...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const startMessage = "sohei big dih";
...
// NOTE: Lanyard can only see your presence if you're a member of the
// Lanyard Discord server: https://discord.gg/lanyard (it just needs to
// share a server with you, that server is the easiest way to enable it).
(function () {
  const DISCORD_USER_ID = '1469330145239367711';
  ...
  if (!avatarEl || !dotEl || !nameEl || !activityEl) return;

  function activityText(presence) {
    if (presence.listening_to_spotify && presence.spotify) {
      return `🎧 ${presence.spotify.song} — ${presence.spotify.artist}`;
    }

    const custom = presence.activities.find(a => a.type === 4);
    if (custom && custom.state) {
      return custom.emoji ? `${custom.state}` : custom.state;
    }
    ...
    if (activity) {
      return `${verbs[activity.type] || 'Playing'} ${activity.name}`;
    }
  ...
  function renderPresence(presence) {
    ...
    if (user.avatar) {
      avatarEl.src = `https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}.${user.avatar.startsWith('a_') ? 'gif' : 'png'}?size=64`;
    }

    dotEl.classList.remove('online', 'idle', 'dnd', 'offline');
    dotEl.classList.add(presence.discord_status || 'offline');

    activityEl.textContent = activityText(presence);
  }
  ...
  // Grab an initial snapshot via REST too, in case the socket is slow to connect
  fetch(`https://api.lanyard.rest/v1/users/${DISCORD_USER_ID}`)
    .then(res => res.json())
    .then(json => {
      if (json.success) renderPresence(json.data);
    })
    .catch(() => {
